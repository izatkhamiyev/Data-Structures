...
public class Fibonacci {
	...
	private static Map<Integer, Long> ansMap = new LLQHashTableMap(100000);
	...
	public static long fibCalc2(int n) {
		...
		callCount2++;
		if(n>1){
                    if(ansMap.getValue(n-1)!=null && ansMap.getValue(n-2)!=null){
                        ansMap.define(n,ansMap.getValue(n-1)+ansMap.getValue(n-2));
                        return ansMap.getValue(n-1)+ansMap.getValue(n-2);
                    }else{
                        if(ansMap.getValue(n-1)!=null){
                            ansMap.define(n,ansMap.getValue(n-1)+fibCalc2(n-2));
                            return ansMap.getValue(n-1)+fibCalc2(n-2);
                        }
                        if(ansMap.getValue(n-2)!=null){
                            ansMap.define(n,ansMap.getValue(n-2)+fibCalc2(n-1));
                            return ansMap.getValue(n-2)+fibCalc2(n-1);
                        }
